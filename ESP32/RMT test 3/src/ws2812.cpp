...
void ws2812_copy()
{
  ...
  for (i = 0; i < len; i++) 
  {
    ...
    for (j = 0; j < 8; j++, bit <<= 1)
    {
      RMTMEM.chan[RMTCHANNEL].data32[j + i * 8 + offset].val = ws2812_bits[(bit >> 7) & 0x01].val;
    }
...
void ws2812_setColors(unsigned int length, rmt_item32_t *array)
{
  ws2812_len = (length) * sizeof(rmt_item32_t);
  rmt_item32_t* ws2812_buffer = (rmt_item32_t*) malloc(ws2812_len);
